...
/**
  ...
  async decideAutoAction(cat) {
    ...
    const prompt = `你是${cat.name}，一只${cat.personality}的猫咪。当前精力${cat.stats.energy}，心情${cat.stats.mood}，饱食${cat.stats.hunger}。
你想做什么？(sleep/play/explore/request) 输出JSON: {"action":"行为","description":"描述","notifyOwner":false}`;
