...
def index():
    ...
    # 每次都重新读取文件（开发模式）
    import os
    file_mtime = os.path.getmtime(FRONTEND_INDEX_FILE)
    if _INDEX_HTML_CACHE is None or _INDEX_HTML_CACHE_TIME < file_mtime:
        ...
        _INDEX_HTML_CACHE_TIME = file_mtime
...
def set_state_endpoint():
    """Set state via POST (for UI control panel or cat-pet simulator)"""
    try:
        data = request.get_json()
        if not isinstance(data, dict):
...
# 猫咪聊天消息存储
CAT_CHAT_FILE = os.path.join(ROOT_DIR, "cat-chat.json")

def load_cat_chat():
    """加载猫咪聊天历史"""
    try:
        if os.path.exists(CAT_CHAT_FILE):
            with open(CAT_CHAT_FILE, "r", encoding="utf-8") as f:
                return json.load(f)
    except Exception:
        pass
    return {"messages": [], "pending_responses": []}

def save_cat_chat(chat_data):
    """保存猫咪聊天历史"""
    try:
        with open(CAT_CHAT_FILE, "w", encoding="utf-8") as f:
            json.dump(chat_data, f, ensure_ascii=False, indent=2)
    except Exception:
        pass

@app.route("/cat-chat", methods=["POST"])
def cat_chat_endpoint():
    """猫咪聊天接口 - 接收用户消息，返回待处理的响应"""
    try:
        data = request.get_json()
        if not isinstance(data, dict):
            return jsonify({"ok": False, "msg": "invalid json"}), 400
...
@app.route("/cat-chat/push", methods=["POST"])
def cat_chat_push_endpoint():
    """推送猫咪响应（供模拟器使用）"""
    try:
        data = request.get_json()
        if not isinstance(data, dict):
        ...
        if not response:
            return jsonify({"ok": False, "msg": "empty response"}), 400

        msg_type = data.get("type", "cat")  # 支持 'user' | 'cat' | 'system'

        # 加载聊天历史
        chat_data = load_cat_chat()

        # 只有猫咪消息才放入待处理队列（用户消息不应被当作猫咪回复弹出）
        if msg_type == "cat":
            if "pending_responses" not in chat_data:
                chat_data["pending_responses"] = []
            chat_data["pending_responses"].append(response)
...
@app.route("/cat-chat/history", methods=["GET"])
def cat_chat_history_endpoint():
    """获取聊天历史"""
    chat_data = load_cat_chat()
    return jsonify({"ok": True, "messages": chat_data.get("messages", [])[-50:]})
