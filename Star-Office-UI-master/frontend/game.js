...
// 更新猫咪状态面板
function updateCatStatusPanel(data) {
  // 更新右上角悬浮面板
  const panel = document.getElementById('cat-status-panel');
  if (panel && data.cat && data.cat.name) {
    panel.style.display = 'block';
    
    const nameEl = document.getElementById('cat-name-text');
    if (nameEl) nameEl.textContent = data.cat.name;
    
    const stats = data.cat;
    ['energy', 'mood', 'hunger', 'cleanliness'].forEach(stat => {
      const el = document.getElementById('cat-' + stat);
      if (el && stats[stat] !== undefined) {
  ...
  ['energy', 'mood', 'hunger', 'cleanliness'].forEach(stat => {
    ...
    if (el && data.cat && data.cat[stat] !== undefined) {
...
// ========== 猫咪聊天窗口功能 ==========
let catChatCollapsed = false;
let catChatMessages = [];

// 切换聊天窗口折叠状态
function toggleCatChat() {
  const panel = document.getElementById('cat-chat-panel');
  const btn = document.getElementById('chat-toggle-btn');
  catChatCollapsed = !catChatCollapsed;
...
function addChatMessage(type, content) {
  const container = document.getElementById('cat-chat-messages');
  const now = new Date();
  ...
  // 保存消息历史
  catChatMessages.push({ type, content, time: timeStr });
}
...
// 从外部添加猫咪消息（供后端推送使用）
function addCatResponse(message) {
  addChatMessage('cat', message);
}

// 更新聊天窗口中的猫咪名字
function updateChatCatName(name) {
  const nameEl = document.getElementById('chat-cat-name');
  if (nameEl && name) {
